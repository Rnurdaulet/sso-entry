...
def get_user_id(iin: str, kc: KeycloakAdmin) -> str | None:
    try:
...
def create_user(iin: str, full_name: str, email: str | None, kc: KeycloakAdmin) -> str | None:
...
def bind_user_to_idp(user_id: str, iin: str, kc: KeycloakAdmin):
    try:
...
def assign_roles(user_id: str, role_names: list[str], kc: KeycloakAdmin):
    try:
...
def create_or_get_user(iin: str, full_name: str) -> str | None:
    ...
    if user_id:
        logger.info(f"[keycloak] Пользователь {iin} найден (ID={user_id})")
        return user_id

    # 2. Получаем данные из внешнего API
